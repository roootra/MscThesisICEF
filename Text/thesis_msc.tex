...
The purpose of this work is to estimate shock-dependent exchange rate pass-through for Russian economy using structural vector autoregression, closing a gap in the literature. I include five variables from 2003Q2 to 2020Q4 into the model: oil price, short-term interest rate for Russia, ruble nominal effective exchange rate, Russian real GDP and CPI in order to calculate price-to-exchange rate ratio for five structural shocks: global persistent shock, local monetary shock, local exogenous exchange rate shock, local supply shock and local demand shock. I use quarterly data in order to estimate the model since there is no monthly time series for Russian output.
...
My findings are comparable to (\cite{Khotulev2020}) for monetary and global persistent (oil) shocks. Pass-through estimate from purely exogenous exchange rate shock is comparable to shock-independent ones captured in the literature about Russia and CIS. Analysis reveals highest pass-through estimate from monetary shocks, while oil shocks, which are commonly considered as a main driver of exchange rate for Russia, have a little pass-through effect. 

Another result that may be interesting for a reader is historical decomposition of Russian CPI growth rate, which shows contribution of different macrovariables in the model to inflation volatility. \textbf{TO BE FILLED.}

As a robustness test, I apply the same algorithm excluding zero restrictions. \textbf{TO BE FILLED.}
...
I estimate vector autoregression (VAR) with five variables: oil price in US dollars, local short-term interest rate (MIACR\footnote{Moscow Interbank Average Credit Rate.} 31-180 days), local nominal effective exchange rate, local real GDP index and local consumer price index (CPI). All variables, except interest rate, are used in growth rates in the model.
...
I use quarterly time series of five variables from 2003Q2 to 2020Q4: oil price in USD, short-term interest rate (MIACR 31-180 days), nominal effective exchange rate, real GDP and CPI. Monthly data are available for all the variables, except for GDP, while use of output proxies seems unreasonable as it may distort final results, as inflation-output and exchange rate-output relations are hardly identifiable.
...
\begin{table}[b!]
	...
	\begin{tabular}{@{}lll@{}}
		...
		Inflation & \makecell[l]{Consumer price index, QoQ, \\seasonally adjusted }          & Rosstat   \\ \bottomrule
	\end{tabular}%
	%}
...
To obtain the results, I estimate BVAR with Normal Inverse-Whishart prior of order 1. The order of model is picked based on SIC (Schwartz information criterion) calculated for models up to 6 lags.

After BVAR estimation, sign and zero restrictions algorithm is applied in order to recover orthogonal impulse-response functions. 466,6 thousands of models are checked and 43 of them fulfil restrictions. As a robustness test, I apply recursive (Cholesky) identification scheme. This approach is more rigid; hence, a reader shouldn't expect this method to be more precise than the first scheme, though the obtained results are comparable.

PERR coefficients are calculated for two time periods: the short-run (4 months) and the long-run one (12 months). For purposes of positive analysis, historical decompositions for CPI and GDP are calculated.

Tables \ref{tab:perr_shortrun} and \ref{tab:perr_longrun} provide PERR estimations. Shock-dependent pass-through coefficients calculated from smooth transition scheme are incomplete for all the variables, except global demand and output shocks. The latter shock is substantial for the economy, as even small output fluctuations generate huge deviations of other macroeconomic variables. Hence, this result might be the case for Russian economy. On the same time, there is a huge difference of results from the recursive scheme. I don't expect PERR for output from Cholesky decomposition to be robust as output shocks have influence only to CPI (due to variable order), while in reality there may be more effects to other variables.
...
Global persistent shock, associated with oil price, is minor in terms of PERR (0.12--0.13), though comparable to one in (\cite{Khotulev2020}). However, it is unlikely this shock happens isolated, as it is usually associated with exchange rate shock.
...
IRFs for smooth covariance transition identification scheme are provided in Annex (Figure \ref{fig:irf_st_1} and \ref{fig:irf_st_2}). 
...
\begin{table}[t]
	...
	%\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}lll@{}}
			\toprule
			Shock                     & Sign and zero & Cholesky  \\
			\midrule
			Global persistent shock   & 0.0695      & 0.1354        \\
			Monetary shock            & 0.0439    & 0.4229        \\
			Exchange rate shock & 0.1459     & 0.2871        \\
			Supply shock              & -0.2389     & 0.0664    \\  
			Demand shock              & -0.0366      & 0.0664    \\   
			\bottomrule
		\end{tabular}%
	%}
	\caption{PERR calculated from BVAR with sign and zero restrictions and VAR with Cholesky decomposition, short-run (2 quarters).}
...
\begin{table}[t]
	...
	%\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}lll@{}}
			...
			Shock                     & Sign and zero & Cholesky  \\
			\midrule
			Global persistent shock   & 0.1584      & 0.1354        \\
			Monetary shock            & 0.2813    & 0.4229        \\
			Exchange rate shock & 0.4033     & 0.2871        \\
			Supply shock              & -0.3556      & 0.0664    \\  
			Demand shock              & -0.1326      & 0.0664    \\   
			\bottomrule
		\end{tabular}%
	%}
	\caption{PERR calculated from BVAR with sign and zero restrictions and VAR with Cholesky decomposition, long-run (4 quarters).}
	\label{tab:perr_longrun}
\end{table}

Figure \ref{fig:hd_cut} contains historical decomposition for CPI growth rate for observations from 2017. See Figure \ref{fig:hd_full} in Annex for full historical decomposition graph.
...
\addcontentsline{toc}{section}{SECTION 6. DISCUSSION}
...
\begin{sidewaysfigure}[h!]
	...
	\caption[]{Impulse response functions for zero and sign identification scheme (1).}
	\label{fig:irf_1}
\end{sidewaysfigure}
...
\begin{sidewaysfigure}[h!]
	...
	\caption[]{Impulse response functions for zero and sign identification scheme (2).}
	\label{fig:irf_2}
\end{sidewaysfigure}
