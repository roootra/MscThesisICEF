...
data_to_fit <- dat_unseas[,c("oil_USD", "imp_price_mom", "miacr_31", 
                           "neer_mom", "output_gap", "cpi_all_mom")]
...
gdp_order = which(colnames(data_to_fit) == "output_gap")
...
bvar_irf_ident_mat[miacr_order,gdp_order] <- NA #gdp?
bvar_irf_ident_mat[miacr_order,cpi_order] <- -1 #cpi declines

#GDP
bvar_irf_ident_mat[gdp_order,oil_order] <- NA #no oil influence
bvar_irf_ident_mat[gdp_order,impprice_order] <- NA #no influence to imp. price
...
VARselect(data_to_fit)$selection
bvar_model <- bvar(data=data_to_fit, lags=2)
bvar_irf_results <- irf(bvar_model, bv_irf(horizon=12, identification = TRUE,
                                           sign_restr=bvar_irf_ident_mat, sign_lim = 500))
