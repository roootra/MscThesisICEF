            ...
            while kk <= nVars && isSignsFulfilled == 1
                ...
                if opt.isNoTransform
                    ...
                    for strkk = searchOrder(1:searchNumber(kk),kk)'
                        % Determine first how many sign restrictions are
                        % used for each variable of the selected shock
                        SignResTemp = mat2cell(S(:,:,strkk),size(S(:,:,strkk),1),repmat(nVars,1,opt.nMaxSignHorizon+1));
                        SignResTemp = vertcat(SignResTemp{:,:});
                        SignResVar = abs(sum(SignResTemp,1))';
                        SignResVar = SignResVar(SignResVar~=0);
                        ...
                        % by construction)
                        if sum(any(S(:,:,strkk))) > 1 && min(sum(cell2mat(cellfun(@transpose,mat2cell(repmat(S(1:sum(SignResVar),:,strkk)*transformedModel.fA*Q(:,kk)-SN(1:sum(SignResVar),:,strkk)*ones(size(S(:,:,strkk),2),1),1,opt.nMaxSignHorizon+1),SignResVar,opt.nMaxSignHorizon+1),'UniformOutput',false)),2)) >= 0
                            ...
                            % updated)
                            break
                        elseif opt.checkNegativeQ && sum(any(S(:,:,strkk))) > 1 && max(sum(cell2mat(cellfun(@transpose,mat2cell(repmat(S(1:sum(SignResVar),:,strkk)*transformedModel.fA*Q(:,kk)+SN(1:sum(SignResVar),:,strkk)*ones(size(S(:,:,strkk),2),1),1,opt.nMaxSignHorizon+1),SignResVar,opt.nMaxSignHorizon+1),'UniformOutput',false)),2)) <= 0
                        ...
                        elseif sum(any(S(:,:,strkk))) <= 1
                            ...
                            tempSign=1;
                            if max(S(:,:,strkk)*transformedModel.fA*Q(:,kk)+SN(:,:,strkk)*ones(size(S(:,:,strkk),2),1)) <= 0
                                tempSign=-1;
                            end
                            if isShockIdent(strkk)==1
