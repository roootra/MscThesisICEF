...
opt.modelName = 'Model_PERR_Q_govexp';     
...
opt.lVars = {'gdpRealQoq', 'cpiAllQoq', 'govExpQoq', 'miacr31','neerQoq', 'oilUSDQoq'}; %a-la Forbes et al. 2018
...
opt.lShocks = {'Supply shock', 'Demand shock', 'Govt. exp. shock', 'Monetary shock', 'Exchange rate shock', 'Oil price shock'};
...
S(1,2,2) = 1
S(6,8,2) = 1
S(2,4,2) = 1 %interest rate response
S(3,10,2) = 1
Z(1,6 + nVars*0,2) = 1
%NEW
S(4,1,2) = 1 %output response
S(5,7,2) = 1
S(6,5,2) = -1 %NEER response (this)
S(7,11,2) = -1
...
S(2,9,3) = 1
Z(1,6 + nVars*0,3) = 1
S(3,4,3) = -1 %interest rate response
S(4,10,3) = -1
S(5,1,3) = -1 %gdp response
S(6,7,3) = -1

%Monetary shock
S(1,4,4) = 1
S(8,10,4) = 1
S(2,1,4) = -1 %output response
S(3,2,4) = -1 %inflation response
S(4,7,4) = -1 
S(5,8,4) = -1
Z(1,6 + nVars*0,4) = 1
%Z(2,5 + nVars*1,3) = 1
...
%Z(4,5 + nVars*3,4) = 1
%NEW
S(4,4,5) = 1 %interest rate response
S(5,10,5) = 1
...
%%% PERR Calculation
perr_gdp = sum(output.irfCTM(1:4,7))/sum(output.irfCTM(1:4,25));
perr_cpi = sum(output.irfCTM(1:4,8))/sum(output.irfCTM(1:4,26));
perr_govexp = sum(output.irfCTM(1:4,9))/sum(output.irfCTM(1:4,27));
perr_int = sum(output.irfCTM(1:4,10))/sum(output.irfCTM(1:4,28));
perr_neer = sum(output.irfCTM(1:4,11))/sum(output.irfCTM(1:4,29));
perr_oil = sum(output.irfCTM(1:4,12))/sum(output.irfCTM(1:4,30));

sprintf("PERRs:\n")
sprintf("Long-term (4q):\n")
sprintf("Oil shock: %0.5f", perr_oil)
sprintf("Monetary shock: %0.5f", perr_int)
sprintf("Exchange rate shock: %0.5f", perr_neer)
sprintf("Government expoenditure shcok: %0.5f", perr_govexp)
sprintf("Output shock: %0.5f", perr_gdp)
sprintf("Inflation shock: %0.5f", perr_cpi)

perr_gdp_s = sum(output.irfCTM(1,7))/sum(output.irfCTM(1,25));
perr_cpi_s = sum(output.irfCTM(1,8))/sum(output.irfCTM(1,26));
perr_govexp_s = sum(output.irfCTM(1,9))/sum(output.irfCTM(1,27));
perr_int_s = sum(output.irfCTM(1,10))/sum(output.irfCTM(1,28));
perr_neer_s = sum(output.irfCTM(1,11))/sum(output.irfCTM(1,29));
perr_oil_s = sum(output.irfCTM(1,12))/sum(output.irfCTM(1,30));

sprintf("Short-term (1q):\n");
sprintf("Oil shock: %0.5f", perr_oil_s);
sprintf("Monetary shock: %0.5f", perr_int_s);
sprintf("Exchange rate shock: %0.5f", perr_neer_s);
sprintf("Government expoenditure shcok: %0.5f", perr_govexp_s);
