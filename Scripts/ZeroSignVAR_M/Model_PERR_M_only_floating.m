...
opt.modelName = 'Model_PERR_M_only_floating';     
...
opt.lShocks = {'Global persistent shock', 'Import price shock', 'Monetary shock', 'Ex. rate shock', 'Aggregate supply shock', 'Aggregate demand shock'};

% % Define start and end date (MM-DD-YYYY).
opt.startDate = '12-01-2014';
...
y = xlsread('data_sign_and_zero_mon_floating.xlsx', 'A1:F186')
[t,nVars] = size(y);
...
opt.nModelDraws = 500;
opt.nDrawsFromBvar = 100;
opt.nTransformationsPerDraw = 10000;
opt.nMatches = 1000;
opt.isIRFcum = 0;
%opt.isNoTransform = 1;
signrestrictions
...
%%% IRF Calculation
irf_oil = sum(output.irfCTM(1:12,19))/sum(output.irfCTM(1:12,31));
irf_imp = sum(output.irfCTM(1:12,20))/sum(output.irfCTM(1:12,32));
irf_int = sum(output.irfCTM(1:12,21))/sum(output.irfCTM(1:12,33));
irf_neer = sum(output.irfCTM(1:12,22))/sum(output.irfCTM(1:12,34));
irf_gdp = sum(output.irfCTM(1:12,23))/sum(output.irfCTM(1:12,35));
irf_cpi = sum(output.irfCTM(1:12,24))/sum(output.irfCTM(1:12,36));
